...
def encrypt():
    ...
    if (key.get() >= chr(65) and key.get() <= chr(90) or key.get() >= chr(97) and key.get() <= chr(122)):
        ...
        ciphertext = ""
        newkeyinput = ""
        #panjang kunci harus >= plaintext
        if len(keyinput) < len(plaintextinput):
            messagebox.showerror("Error", "Key requires more letters")
        else: #panjang kunci >= plaintext
            for i in range (len(plaintextinput)):
                newkeyinput = newkeyinput + chr(ord(keyinput[i])) #pakai key sepanjang plaintext
            for i in range(len(plaintextinput)):
                ciphertext += chr(((ord(plaintextinput[i]) - 65) + (ord(newkeyinput[i]) - 65)) % 26 + 65)
    ...
    if len(key.get()) == 0:
        ...
        for i in range(len(ciphertext)):
            if(ciphertext[i] >= chr(65) and ciphertext[i] <= chr(90)):
                plaintextinput += chr(((ord(ciphertext[i]) - 65) - (ord(keyinput[i]) - 65)) % 26 + 65)
            else:
                continue 
...
#untuk membuka file plainteks dan langsung ada di kotak teks
def openfiletxt():
...
#untuk membuka file plaintext nonteks (biner)
def openfilebiner():
...
#untuk membuka file kunci dan langsung ada di kotak kunci
def openfilekunci():
    file = filedialog.askopenfile(mode='r', filetypes=[('Text files', 'txt')])
    if file is not None:
        content = file.read(10000)
...
entry3.grid(row=1, column=4, sticky=W, padx=5, pady=5)
...
label5.grid(row=2, column=3, sticky=W, padx=5, pady=5)
entry5 = Entry(root, textvariable=cipherfive)
entry5.grid(row=2, column=4, sticky=W, padx=5, pady=5)
...
button3.grid(row=1, column=2, sticky=W, padx=5, pady=5)

button4 = Button(root, text="Open Binary File", command=openfilebiner)
button4.grid(row=2, column=2, sticky=W, padx=5, pady=5)
...
button6.grid(row=4, column=0, sticky=W, padx=5, pady=5)
